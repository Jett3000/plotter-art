...
function draw() {
  ...
  for (let currLine = 0; currLine <= lineCount; currLine++) {
    let verticalProgress = pow(currLine / lineCount, 1.9);
    for (let currSeg = 0; currSeg < segmentCount; currSeg++) {
      vertex(x, y + processedNoise(x, y, nd) * verticalProgress);
...
function processedNoise(x, y, nd) {
  return (pow(noise(x * nd.x), 3) - 0.5) * nd.mag;
}
