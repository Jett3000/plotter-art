...
function draw() {
  ...
  let factors =
    [2, 3, 4, 5, 6,
      ...
      40, 45, 60, 72, 90,
      120, 180, 360];
  let stepsPerPeriod = 15; //random(factors)


  outerPoints.forEach((p, i) => {
    let length = i % stepsPerPeriod;
    let halfPeriod = floor(stepsPerPeriod / 2)
    length = abs(length - halfPeriod) / halfPeriod / 2;
    // if (length == 0) return;
    ...
    inverseLength = ease(inverseLength);
    let outerPoint = p5.Vector.mult(p, 1 + length / 4);

    line(outerPoint.x, outerPoint.y, p.x, p.y);
  });
