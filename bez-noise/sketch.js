...
function setup() {
  ...
  createCanvas(windowWidth, windowHeight * 0.2, SVG);
  noFill();
  strokeWeight(0.8);
  ...
  for (let col = 0; col < cols; col++) {
    for (let row = 0; row < rows; row++) {
      ...
      let pos = createVector(
          ...
          lerp(0, height, (0.5 + row) / rows) + 5 * sin(8 * TWO_PI * progress));
...
class BCurve {
  ...
  show() {
    ...
    let angle = noise(this.pos.x * noiseSlider.value()) * TAU *
        noiseParams.mag * this.progress;
